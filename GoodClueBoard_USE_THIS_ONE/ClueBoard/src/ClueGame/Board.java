...
public class Board {
	...
	private int numRows, numColumns, initialLocation;
	...
	public void calcAdjacencies() {
		...
		for(int key : keys) {
			...
			tempRow = key/(numColumns);
			tempCol = key%(numColumns);

			if(tempRow < numRows && tempCol < numColumns) {
				int index = calcIndex(tempRow, tempCol);
				boolean atEdge = false;
				BoardCell current = null;
				BoardCell previous = cells.get(index);
				...
				if(!atEdge && ((current.isWalkway() && (!previous.isRoom() || previous.isDoorway())) || 
	...
	public void startTargets(int row, int column, int steps) {
		int index = calcIndex(row,column);
		calcTargets(row, column, steps, index, 0);
	}

	//calculates the possible targets the given number of steps away, starting
	//from the given row and column
	public void calcTargets(int row, int column, int steps, int initial, int recursed) {
		int index = calcIndex(row,column);
		//		setVisited();
		...
		if(steps != 0 && getCellAt(index).isWalkway()){
			...
			for(Integer i : temp){
				if (!visited[i]){
					if(recursed == 0 || index != initial) {
						calcTargets(i/numColumns, i%numColumns , steps -1, initial, 1);
						setVisited();
					}
				}
			}
		...
		else if(steps == 0 || (getCellAt(index).isDoorway() || !getCellAt(index).isRoom())){
			if((index != initial && !visited[index]) && (getCellAt(index).isDoorway() || steps == 0)){
